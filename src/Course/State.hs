...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> State sa = State $ \s -> fstMap f $ sa s
...
findM mp (x :. xs) = (\p -> if p then return $ Full x else findM mp xs) =<< mp x
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval st S.empty
  where
    st = findM (\x -> State $ \s -> (S.member x s, S.insert x s)) xs
...
distinct xs = eval st S.empty
  where
    st = filtering (\x -> State $ \s -> (not $ S.member x s, S.insert x s)) xs
...
squareSum i = toInteger . sum $ map (\x -> digitToInt x P.^ 2) $ show' i

isHappy ::
  Integer
  -> Bool
isHappy = contains 1 . firstRepeat . produce squareSum
