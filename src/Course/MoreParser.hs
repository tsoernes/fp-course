...
hex ::
  Parser Char
hex = (\hs -> case readHex hs of
  Full hs' -> valueParser $ chr hs'
  Empty    -> failed) =<< replicateA 4 (satisfy isHexDigit)
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = (\a -> (\xs -> valueParser (a:.xs)) =<< list (ps >>> pa)) =<< pa
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r pa = betweenCharTok l r $ sepby pa commaTok
